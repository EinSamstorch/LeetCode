...
class Solution {
    public boolean circularArrayLoop(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            // 慢指针slow从i的位置开始移动（类似slow = node)，快指针fast从i移动一次以后的位置开始移动（类似fast = node.next)
            int slow = i;
            int fast = next(nums, i);

            // 保证快慢指针始终向着nums[i]表示的同一个方向移动，> 0始终向右，反之始终向左
            while (nums[i] * nums[slow] > 0 && nums[i] * nums[fast] > 0 && nums[i] * nums[next(nums, fast)] > 0) {
                // 快慢指针相遇，在循环中相遇，类似于在环形链表里相遇
                if (slow == fast) {
                    if (fast == next(nums, fast)) {
                        // 循环长度为1，不能为true
                        break;
                    }
                    return true;
                }
                // 移动慢指针，类似slow = slow.next
                slow = next(nums, slow);
                // 移动快指针, 类似fast = fast.next.next
    ...
    private int next(int[] nums, int i) {
        ...
        if (next >= 0) {
            ...
            // 这边为了防止负数过大，还可以将nums.length 乘以一个较大的正整数.
